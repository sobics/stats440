...
for(i in 1: 100) {
  ...
  p_finmod <- predict(finmod, dat)
  R4 <- rbind(R4, RMSE(p_finmod, test$wblot))
}
ggplot(data = dat, aes(p_finmod, wblot)) + geom_point(aes(colour = lab)) + geom_smooth() #+ geom_line(aes(colour=lab), linetype=5)
...
med_p <- ggplot(test,aes(p_finmod)) + geom_histogram() + lims(x = c(0,4)) + theme_bw()
...
InHee Ho - explored various models, edited case study 2
