...
```
Looking at the Residuals vs. Fitted plot, the points exhibit homoscedascity, and overall, the points and randomly and evenly distributed above and below the horizontal axis. This implies that the data should be modeled linearly. 
```{r}
qqnorm(resid(fm))
```
...
However, although a good indicator, it is important to note that relying solely on RMSE to assess the goodness-of-fit of a model is not enough; we must rely on multiple measurements like AIC, Adjusted R-Squared, ANOVA tests, etc to compare models. 
###Test & Training Data
We decided to split the cleaned dataset from our first write-up into test and training data. We fit the model on the training data to see how well it fits our test data. Our test data is 1/10 of the full dataset. We sampled/fit the test and training data for 100 iterations for each model in an effort to normalize the performance of the models.
```{r, cache = T}
R4 <- NULL
for(i in 1: 100) {
  ...
  R4 <- rbind(R4, RMSE(exp(p_finmod)-1, test$blot))
}
```
