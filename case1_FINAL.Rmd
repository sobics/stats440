...
[show plot of dose1 and discuss the high leverage of the outliers] It is critical to counter the leverage in our regression model - we use $log(dose1)^2$ in our model to fix this. After doing the log of dose1, we notice that it is not exactly linear and include a polynomial to better fit the model.
```{r}
...
ggplot(data=dat, aes(dose1)) + geom_histogram()
ggplot(data = dat, aes(dose2)) + geom_histogram()
