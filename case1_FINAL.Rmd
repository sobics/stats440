...
#Data
Our final dataset contains 2677 observations and 6 features: lab, protocol, dose 1, dose 2, body weight, and blotted uterus weight.
```{r}
...
#Final Model
Originally the response varaible was simply blot - this was incorrect due to differences in scale in addition to heteroskedasticity in the model. This was corrected in the final model using $log(blot+1)$ as the response variable.
```{r}
...
$$log{(y_{ij}+1)} = \mu_i + \beta_{i,dose1}\cdot log^2(dose1_{ij}+1) + \beta_{i,dose2}\cdot log^2(dose2_{ij}+1) + \gamma_{dose1}\cdot log^2(dose1_{ij}+1) + \gamma_{dose2}\cdot log^2(dose2_{ij}+1) + \gamma_{body}\cdot log^2(body_{ij}) + \gamma_{inter}\cdot proto_{ij}\cdot lab_{ij} +\epsilon_{ij}$$ where $\epsilon_{ij} \sim N(0,\nu^2)$ and $\mu_i \sim N(\mu, \sigma^2)$
