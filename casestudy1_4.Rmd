...
m2 <- lmer(log(blot + 1) ~ (1 + proto + kern_distribution(dose1, knots = 5) + kern_distribution(dose2, knots = 3)|lab) + kern_distribution(dose1, knots = 5) + kern_distribution(dose2, knots = 3) , data = dat)  #X + Z + R + S + random effect
#summary(m2)
...
for(i in 1: 100) {
  ...
  R5 <- rbind(R4, RMSE(p_finmod, log(test$blot+1)))
}
```
