...
smooth_graph <- function(x, y.predict, knots = 4, s = 2) {
...
m2 <- lmer(log(blot + 1) ~ (1 + proto|lab) + kern_distribution(dose1, blot, knots = 3, s = 1) + kern_distribution(dose2, blot, knots = 3, s = 1) + kern_distribution(body, blot, knots = 2, s = 1), data = dat) #X + Z + R + S + random effect
summary(m2)

plot(dat$dose1,log(dat$blot + 1))
