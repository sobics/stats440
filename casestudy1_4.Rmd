...
smooth_graph <- function(x, y.predict, knots = 4) {
  tau <- seq(min(x), max(x), length.out = knots)
  s <- diff(tau)[1]/2
  x.predict <- seq(min(x), max(x), length.out=500)
...
m2 <- lmer(log(blot + 1) ~ (1 + proto|lab) + kern_distribution(dose1, blot, knots = 2) + kern_distribution(dose2, blot, knots = 3) + kern_distribution(body, blot, knots = 3), data = dat) #X + Z + R + S + random effect
summary(m2)
...
m2 <- lmer(log(blot + 1) ~ (1 + proto|lab) + smooth_graph(dose1, blot, knots = 2) + smooth_graph(dose2, blot, knots = 3) + smooth_graph(body, blot, knots = 3), data = dat) #X + Z + R + S + random effect
summary(m2)
y.predict <- predict(m2, dat)
