...
Instead of modeling dose 1 and dose 2 as polynomials with a degree of 2, we decided to capture the non-linearity of dose 1 and dose 2 via kernel regression. 
...
We also noted that body exhibited non-linear trends, so we also decided to model this feature via kernel regression. However, clustering body into 3 segments produced better predictive results.

#Final Model Goodness of Fit
```{r}
m4 <- lmer(log(blot + 1) ~ (1 + proto + kern_distribution(dose1, knots = 5) + kern_distribution(dose2, knots = 3)|lab) + kern_distribution(dose1, knots = 5) + kern_distribution(dose2, knots = 3) + bodyClust, data = dat) #X + Z + R + S + random effect
summary(m4)
```
...
Looking at the residuals vs. fitted, this graph 
